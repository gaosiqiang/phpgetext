...
for($i=$start;$i<$max;$i++)
{
    $response = $http->get('http://bj.nuomi.com/320-page'.$i.'?#j-sort-bar');
    $html = $response;
...
$myfile = fopen("nuomi_bj_yule".$start."-".$max.".text", "w") or die("Unable to open file!");
...
foreach($array as $k => $v1)
{
    ...
    foreach($shop_title as $key => $value)
    {
        ...
        $txt = '("'.$title.'","'.$address.'","'.$tel.'",'.$score.','.$sales.',"娱乐","'.$fl.'"),'."\n";
        fwrite($myfile, $txt);
        echo $txt;
    }
