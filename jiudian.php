...
for($i=$start;$i<$max;$i++)
{
    $response = $http->get('http://v.nuomi.com/-page'.$i.'?#j-sort-bar');
    $html = $response;
...
$myfile = fopen("nuomi_bj_bendi".$start."-".$max.".text", "w") or die("Unable to open file!");
...
//    }
    ...
    foreach($shop_title as $key => $value)
    {
        ...
        $txt = '("'.$title.'","'.$address.'","'.$tel.'",'.$score.','.$sales.',"酒店","酒店"),'."\n";
        fwrite($myfile, $txt);
        echo $txt;
    }
