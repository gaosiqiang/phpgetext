...
for($i=$start;$i<$max;$i++)
{
    $response = $http->get('http://bj.nuomi.com/326-page'.$i.'?#j-sort-bar');
    $html = $response;
...
$myfile = fopen("nuomi_bj_meishi".$start."-".$max.".text", "w") or die("Unable to open file!");
...
//            $sql = 'insert into nuomimeishi (name,address,tel,score,sales,flevel,slevel) VALUES '.$txt;
//            if($mysqli->query($sql))
//            {
//                echo $txt;
//            }else {
...
//    var_dump($shop_title[0]);
//    var_dump($shop_address[0]);
//    $s = strpos($infos,$shop_address[0]->node->nodeValue);
//    $infos = substr($infos, $s);
//    $s = strpos($infos, 'bn-phone=');
//    $infos = substr($infos, $s+10);
//    $n = strpos($infos,'mon');
//    $infos = substr($infos, 0, $n-2);//商户电话
